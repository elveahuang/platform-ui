    ...
    <template v-for="item in props.items" :key="item.id">
        ...
        <van-cell-group inset @click="gotoDetailPage(item.id)">
            <van-cell-group inset>
                <div class="app-text app-text-bold app-text-ellipsis-1 mt-2" v-html="item.title"></div>
                <div class="app-line"></div>
                <div class="app-text-muted app-text-ellipsis-2 mb-4" v-html="item.content"></div>
            </van-cell-group>
        </van-cell-group>
    </template>
    <app-divider />
