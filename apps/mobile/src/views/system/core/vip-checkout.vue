<template>
    <van-sticky>
        <van-nav-bar :border="false" left-arrow @click-left="goBack()">
            <template #title>
                <van-tabs ref="vipTypeTabsRef" v-model:active="vipTypeTabsActive" v-if="vipTypeTabsShow" :line-width="20" @change="vipTypeTabsChange">
                    <van-tab v-for="vipType in vipTypeList" :key="vipType.code">
                        <template #title>
                            <div class="mx-4">{{ vipType.title }}</div>
                        </template>
                    </van-tab>
                </van-tabs>
            </template>
    ...
    >
        <van-cell-group inset class="!my-4">
            <van-cell v-for="item in payTypeList" :key="item.code" is-link>
                ...
                <template #title>
                    <div class="app-text ml-4" @click="submitVipOrder(item.code)">{{ item.title }}</div>
                </template>
            </van-cell>
        </van-cell-group>
    </van-action-sheet>
