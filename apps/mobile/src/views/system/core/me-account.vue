<template>
    ...
    <van-cell-group inset>
        <van-cell :title="t('app.photo')" to="/setting" isLink>
            <van-image class="align-middle" round width="1.5rem" height="1.5rem" src="https://fastly.jsdelivr.net/npm/@vant/assets/cat.jpeg" />
        </van-cell>
        <van-cell :title="t('app.adress')" to="/setting" isLink>
            {{ user.adress }}
        </van-cell>
        <van-cell :title="t('app.name')">
            {{ user.username }}
        </van-cell>
        <van-cell :title="t('app.nickname')" to="/setting" isLink>
            {{ user.displayName }}
        </van-cell>
        <van-cell :title="t('app.role')" to="/setting" isLink>
            {{ user.role }}
        </van-cell>
        <van-cell :title="t('app.email')">
            {{ user.email }}
        </van-cell>
        <van-cell :title="t('app.mobile')" to="/setting" isLink>
            {{ user.mobileNumber }}
        </van-cell>
        <van-cell :title="t('app.signature')" to="/setting" isLink>
            {{ user.signature }}
        </van-cell>
    </van-cell-group>
</template>
