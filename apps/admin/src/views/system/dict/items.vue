<template>
    <app-page-header>
        <template #title
            >{{ $t('common.dict_items_management') }} - <label class="!text-neutral-500">{{ pageTitle }}</label>
        </template>
    </app-page-header>
    ...
    <a-modal class="app-modal" v-model:open="entityFormModalOpen" :title="entityFormModalTitle" @ok="handleFormSubmit" @cancel="handleFormCancel">
        <a-card>
            ...
            >
                ...
                <a-form-item name="title" :label="$t('common.field_label')">
                    <a-input :maxlength="255" v-model:value="entityFormModel['title']" :placeholder="t('common.field_label_placeholder')" />
                </a-form-item>
                <a-form-item name="idx" :label="$t('common.field_idx')">
                    <a-input-number :max="999" :min="0" v-model:value="entityFormModel['idx']" :placeholder="t('common.field_idx_placeholder')" />
                </a-form-item>
            </a-form>
        </a-card>
    </a-modal>
...
import {
    checkItemCodeApi,
    dictItemEntityDeleteApi,
    dictItemEntityDetailsApi,
    dictItemEntityPageApi,
    dictItemEntitySaveApi,
...
const getDataList = async () => {
    await dictItemEntityPageApi({ typeId: typeId, ...handleParams(dataTableModel) }).then((result) => {
...
let entityFormModel = reactive<DictItemEntity>(new DictItemEntity());
const entityFormRules: Record<string, Rule[]> = {
    code: [
        {
            validator: async (rule: Rule, payload: { code: string; id: number | string }): Promise<void> => {
                const result: R<Boolean> = await checkItemCodeApi({ ...payload });
            ...
            transform: (value: string): { code: string; id: number | string } => {
...
const showEntityFormModal = async (id?: number) => {
    ...
    if (id && id > 0) {
        entityFormModalTitle.value = t('common.dict_items_pages_edit_title');
        await dictItemEntityDetailsApi({ id: id }).then((result) => {
            Object.assign(entityFormModel, { ...result.data });
        });
    } else {
        Object.assign(entityFormModel, { ...new DictItemEntity() }); // 重置表单参数
        entityFormModalTitle.value = t('common.dict_items_pages_add_title');
    }
};
...
const handleFormSubmit = async () => {
    entityFormModel.typeId = typeId;
    await entityForm.value
        .validate()
        .then(async () => {
            dictItemEntitySaveApi(entityFormModel).then((result) => {
                if (result.code == '200') {
                    entityFormModalOpen.value = false;
...
const handleDelete = async (id: number = 0) => {
    ...
    if (isEmpty(ids)) {
        ...
        dictItemEntityDeleteApi({ ids: ids }).then((result) => {
            if (result.code == '200') {
                entityFormModalOpen.value = false;
                dataTableRef.value.refresh();
            } else {
...
const showPreviewModal = async (entity: DictItemEntity) => {
    previewContentHtml.value = entity.title;
