<template>
    <a-page-header :title="$t('common.vip_management')" />
    ...
    <!-- 编辑用户 -->
    <edit-account v-model:show="editUserFormModalOpen" v-model:value="editUserFormModalValue" @onSuccess="getDataList" />
    <!-- 修改密码面板 -->
    <reset-password v-model:show="resetPasswordFormModalOpen" v-model:value="resetPasswordFormModalValue" />
    <!-- 添加用户 -->
    <add-account v-model:show="addUserFormModalOpen" @onSuccess="getDataList" />
</template>
...
import { AddAccount, EditAccount, ResetPassword } from '@/views/system/account/components';
import { accountDeleteApi, accountListApi } from '@commons/core/api/admin/account.ts';
...
const dataTableColumns = [
    {
        title: t('common.field_account_name'),
        dataIndex: 'username',
        key: 'username',
        width: 100,
        ellipsis: true,
...
const getDataList = async () => {
    await accountListApi(handleParams(dataTableModel)).then((result) => {
...
const handleDelete = async (id: number = 0) => {
    ...
    accountDeleteApi({ ids: ids }).then((result) => {
        if (result.code == '200') {
