<template>
    <a-modal class="app-modal" v-model:open="open" :title="$t('common.account_pages_edit_title')" @ok="handleSubmit" @cancel="handleCancel">
        <a-card>
            <a-form ref="entityForm" :model="model" :rules="rules" :label-col="{ span: 4 }" :wrapper-col="{ span: 12 }">
                <a-form-item name="username" :label="$t('common.field_account_name')">
                    <label>{{ model.username }}</label>
                </a-form-item>
                <a-form-item name="email" :label="$t('common.field_account_name')">
